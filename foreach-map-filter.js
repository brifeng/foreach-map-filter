...
function showFirstAndLast(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        output.push(`${firstLetter}${lastLetter}`);
    }
    return output;
}
...
function find(arr, searchValue) {
    ...
    if (output.includes(searchValue))
        return searchValue;
    return undefined;
}
...
function findInObj(arr, key, searchValue) {
    ...
    for (let i = 0; i < filteredArray.length; i++) {
        for (const obj in filteredArray[i]) {
            if (filteredArray[i][obj] === searchValue) {
...
function removeVowels(str) {
    const vowels = 'aeiou';
    const output = Array.from(str).filter(function(char) {
        return !Boolean(vowels.indexOf(char.toLowerCase()) + 1);
    })
...
function doubleOddNumbers(arr) {
    const doubledNumArray = arr.map(function(val) {
        return val * 2;
    })
    return doubledNumArray.filter(function(val) {
        return val / 2 % 2 === 1;
    })
}